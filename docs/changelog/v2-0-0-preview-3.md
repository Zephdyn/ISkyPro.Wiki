...
- linux: Linux preview 包支持主程序、WebUI、QQBot 网关和 Plugin SDK v2 新插件；旧 DLL 插件、`isky.exe` 和 `message.dll` 兼容层仍仅支持 Windows 包。
...
- docs: 更新 Wiki 快速开始、FAQ 和 SDK 下载页面，补充 Linux preview 包启动、systemd 长期运行、Linux 自动更新和旧插件 Windows-only 边界。
